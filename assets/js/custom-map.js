...
function createHomepageGoogleMap(_latitude,_longitude,_idMap, _zoom){
    setMapHeight();
    if( document.getElementById('map') != null ){
        $.getScript("../assets/js/locations.js", function(){
            ...
            for (i = 0; i < locations.length; i++) {
                //Validación si se quieren mostrar todas las propiedades o solo
                //las pertenecientes a un sector
                if (_idMap == 0 || _idMap == locations[i][8] || _idMap == locations[i][9]) {
                    ...
                    boxText.innerHTML =
                        '<div class="infobox-inner">' +
                            ...
                            '</div>';
                    //Define the infobox
                    newMarkers[numMarkers].infobox = new InfoBox(infoboxOptions);
                    google.maps.event.addListener(marker, 'click', (function(marker, numMarkers) {
                        return function() {
                            for (h = 0; h < newMarkers.length; h++) {
                                newMarkers[h].infobox.close();
                            }
                            newMarkers[numMarkers].infobox.open(map, this);
                        }
                    })(marker, numMarkers));
                    numMarkers++;
                }
